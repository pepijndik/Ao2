...
abstract class User
{
	...
	static function init($db_drvr, $db_host, $db_name, $db_user, $db_pass)
	{
		try {
			switch ($db_drvr) {
				case 'mysql':
					...
					break;
				case 'mssql':
					self::$_db = new \PDO('mssql:host=' . $db_host . ';dbname=' . $db_name, $db_user, $db_pass);
					break;
				case 'sybase':
					self::$_db = new \PDO('sybase:host=' . $db_host . ';dbname=' . $db_name, $db_user, $db_pass);
					break;
				default:
					self::$_db = new \PDO('mysql:host=' . $db_host . ';dbname=' . $db_name, $db_user, $db_pass);
					break;
		...
		if (!$session_check) {
			die('Session is not started. Session are required for Pdik\User class.');
		}
